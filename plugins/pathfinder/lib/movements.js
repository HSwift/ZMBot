...
class Movements {
  ...
  getMoveForward (node, dir, neighbors) {
    ...
    if (!blockD.liquid && !blockD.physical && !blockC.liquid) { // :swift move to water
      if (node.remainingBlocks === 0) return // not enough blocks to place
  ...
  getUpInWater (node, neighbors) { // :swift add keep up in water movement
    const block1 = this.getBlock(node, 0, 0, 0)
    let waterBlock = this.getBlock(node, 0, 1, 0)

    if(!block1.liquid) return

    while (waterBlock.position) {
      if (waterBlock.name === 'air'){
        if(this.getBlock(waterBlock.position, 0, 1, 0).name === 'air'){
          const cost = (waterBlock.position.y - node.y) * this.liquidCost
          neighbors.push(new Move(node.x, waterBlock.position.y, node.z, 0, cost, [], []))
          return
        }
      }
      if (!waterBlock.liquid) return
      waterBlock = this.getBlock(waterBlock.position, 0, 1, 0)
    }
  }
  ...
  getMoveParkourForward (node, dir, neighbors) {
    ...
    if(block0.liquid) return // swift: can't parkour in water 
  ...
  getNeighbors (node) {
    ...
    this.getUpInWater(node, neighbors) // :swift
